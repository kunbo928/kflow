...
6. **同类聚合与命名**——同 type 文档遵循 `{type}-{slug}.md`，根目录某 type ≥6 份是否还平铺（参照 `shared-paths.md`）
...
```bash
python .kflow/tools/search-yaml.py --dir .kflow/compound --filter "doc_type=decision|explore|learning" --query "{模块关键词}"
python .kflow/tools/search-yaml.py --dir .kflow/compound --filter doc_type=decision --filter status=active --query "{模块关键词}"
```
