...
`k-issue` 等你报 bug，`k-refactor` 等你指优化点，`k-explore` 等你提问题——但"我也不知道哪有问题，你先扫一遍看看"这个诉求没人接。`k-audit` 补上这块：**在用户限定的范围内主动扫描，产出一份按严重度 × 性质交叉分类的发现清单**。

本技能只发现、不定修。修是 `k-issue` / `k-refactor` 的事。
...
另外每个发现带 **置信度**（`high` / `medium` / `low`）和**建议动作**（`k-issue` / `k-refactor`）。
...
- **架构偏离**：代码与 `.kflow/architecture/` 记录不一致、分层泄漏、跨模块隐式耦合
...
用户选哪条 → 路由到 `k-issue` 或 `k-refactor`。`k-audit` 自己不修。
...
| 技能 | 触发 | k-audit 怎么对待 |
|---|---|---|
| `k-issue` | 用户报已知 bug | audit 发现 bug 后建议开 `k-issue` |
| `k-refactor` | 用户指已知优化点 | audit 发现可优化点后建议开 `k-refactor` |
| `k-explore` | 围绕一个问题查代码 | audit 是批量扫多个维度，不等同于 explore |
| `k-arch` | 维护架构文档 | k-arch 维护文档，k-audit 检查代码是否偏离文档 |
| `k-security-review` | 安全审查 | audit 的安全维度是轻量扫描，深度安全审查走专项 |
...
- **只发现不定修**——k-audit 不出代码改动。出现"顺便修了"就算越界
- **架构偏离引用当前文档**——不准凭记忆判断架构应该长什么样，必须读 `.kflow/architecture/` 对照
