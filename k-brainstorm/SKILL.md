---
name: k-brainstorm
description: 想法还模糊时的讨论入口，做分诊后路由到 feature-design / feature-brainstorm / roadmap。采用逐支澄清方法：沿决策树按依赖顺序推进、单题串行并给建议答案，可由代码验证的问题先查代码。AI 是思考伙伴不是记录员。触发：用户说"有个想法还没想清楚"、"先 brainstorm 一下"、"聊一聊这块"、"方向还在摇摆"、"grill me"。不处理 bug 和重构。
---
...
用户自己拆成多件 → 多 feature 规模，追问"想直接拆 roadmap 还是先深挖后存着？"→ case 3 或 case 4；a/b/c 是同一件事的不同面 → case 2；用户听完复述说"对就是这个想清楚了"→ case 1。
...
**逐支澄清方法（case 2 / case 4 默认工作方式）**：

- **目标是共享理解，不是问答数量**——退出标准是关键分支被澄清，而不是问满轮数
- **覆盖每个关键分支**——围绕目标、约束、边界、方案、风险逐支走，不跳枝；某分支未决就不假装已对齐
- **先依赖后细节**——先问"目标/约束/边界"再问"方案/实现"，避免在未对齐目标前陷入实现细节
- **每轮都要有立场**——提问时给 2-4 个有区别度选项，并给推荐答案（含一句理由），让用户确认/修正
- **可验证先验证**——能从仓库事实拿到答案就先查代码，再带着证据回来问，不让用户盲猜现状
- **每轮产出分支状态**——本轮结论必须落在 `confirmed / rejected / open question` 之一，避免原地打转

1. **挖问题**——按姿态 1 把"真正要解决的问题"问清楚，能用一句话复述、用户说"对就是这个"为止。**这一步价值最高不要急着跳过**

   **深挖模式**（按需启动，默认不开）

   默认走轻问——一次复述对上就推进。下面任一信号出现切到深挖模式加深：

   - **显式请求**：用户说"多问几轮 / 帮我问清楚再开始 / grill 我"
   - **隐式信号**：连续两次复述被"差不多但不太对"驳回；同一概念用不同词反复互指（"权限 / 角色 / 租户"换着说指同一件事）；用户自己也说不清楚
   - **只在 case 2 / case 4 启动**——case 1 已清楚再深挖反人性，case 3 用户已 ready 拆解不需要深挖
   - **强度调节**：用户明确说 "grill me" 时，按"关键分支尽量走完"执行；未显式要求时维持轻量深挖，避免过度盘问

   深挖模式硬约束（防止没完没了）：

   - 最多 3-5 轮重点问题，一轮没拿到新增信息就退到发散（轮数是上限不是目标）
   - 每轮**一个问题 + 2-4 个有区别度的候选**让用户挑，不让 TA 自由作文（单题串行）
   - 每轮问题后都给**推荐答案**（先说建议，再让用户确认/修正）
   - 某问题如果能靠仓库事实回答（接口现状、行为约束、已有实现），先读代码再回来问，不把可验证问题丢给用户猜
   - 分支推进按依赖顺序逐个解决：上游分支未确认，不进入下游实现分支
   - 每轮结束时标记本轮分支状态：`confirmed / rejected / open question`；连续两轮都只产生 open question 就退到发散或提议 spike
...
3. **成本可控**——你判断 5-30 分钟内能搭出能跑的东西。超过这个量该走 `k-feat-ff` 直接做或拆成正式 feature
...
**处理**：
1. 告诉用户"这块你已经想清楚了：{AI 一句话复述}。建议直接 `k-feat-design`——brainstorm 对你没增量"
2. **看聊过程有没有非琐碎技术决策**——讨论了具体库选型 / Schema / 接口形态 / 跨模块约定，落一份精简 brainstorm（只填"已敲定的设计点"那节）让 design 直接读到不必重讨；纯方向确认没聊技术细节就裸退不落盘
3. 停下来等用户触发 design

**退出**："直接触发 `k-feat-design` 从零写 design"（不落盘）；轻量落盘则"下一步 `k-feat-design` 会读到 `{路径}` 不必重述"
...
**处理**：
1. 告诉用户"听起来是多个 feature 的集合，单 feature 装不下。`k-roadmap` 会做拆解和依赖梳理，我把讨论交给它"
...
**怎么聊**：启动深挖模式（见上节"对话节奏 > 深挖模式"），同时自由发散——聊方案、聊类比、聊技术可能性、聊限制。对话比 case 2 更开放，不急着收敛。

**升降级**：
- 深挖完感觉够清楚了想直接拆 → case 3，落 brainstorm.md 后移交 roadmap
...
2. **不替用户决定规模**——case 2 / 3 / 4 边界模糊就问用户"你脑子里这块是一个 feature 能装下的规模吗，还是需要先深挖后存着"
3. **不落盘非 case 2 / case 4 产物**——case 1 / 3 不写文件
4. **不处理 bug / 重构**
5. **不在 case 1 / 3 启动深挖模式**——case 1 已清楚再深挖反人性，case 3 用户已 ready 拆解不需要深挖
6. **别自己顺手开始写 design 或 roadmap**——阶段间的人工 checkpoint 是 kflow 整套流程的硬约束
...
- 把 case 4 当 case 3 处理——用户想先深挖并存着，却直接移交 roadmap 把未成形的想法硬拆成 feature
- 把 case 3 当 case 4 处理——用户已经 ready 拆解，却强行深挖拖延节奏
