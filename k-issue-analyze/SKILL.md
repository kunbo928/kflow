...
1. **问题报告存在且已确认**——读 `{slug}-report.md`，确认 `doc_type=issue-report` 且 `status=confirmed`，5 节都有内容。不完整 / 状态不对 → 回 `k-issue-report`。`k-issue-report` 已判走标准路径就按标准路径走，不二次改判
...
- [ ] 5 节都填完
- [ ] 定位到具体代码位置（`{文件}:{行号}`）
- [ ] 根因处附核心原始代码片段（3-10 行，含 `file:line`）
- [ ] 失败路径还原清晰
- [ ] 影响面评估完成
- [ ] 至少 2 种修复方案 + 推荐
- [ ] 用户明确确认"分析准确，用方案 X 修"
...
告诉用户："根因分析已就绪，方案已确认。下一步阶段 3 修复验证，触发 `k-issue-fix`。"
...
- 根因写"可能是某处的问题"——必须定位到 file:line，并给出最核心原始代码片段
