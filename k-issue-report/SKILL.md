...
1. **确认是 bug 不是新功能需求**——描述"想加 X 功能"的告诉他走 `k-feat`
2. **看有没有相关 issue 目录**——Glob `..kflow/issues/`，有同类问题先和用户确认是新建还是更新
3. **快速通道判断（唯一正式判定点）**——按用户线索**读一下相关代码**（Grep / Read 定位）：
   - **能一眼确定根因**（能给出 `{文件}:{行号}`、修复改动小 1-2 处、无跨模块影响风险）→ 告诉用户"我已看到问题所在，可以走快速通道：直接告知根因和修复方案，你确认后我立刻修，修完你验证，只写一份 `{slug}-fix-note.md`"。同意后触发 `k-issue-fix`（快速通道模式）
...
告诉用户："问题报告已就绪。下一步阶段 2 根因分析，触发 `k-issue-analyze`。"
