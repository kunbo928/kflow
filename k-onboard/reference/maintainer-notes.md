...
本文件由 `k-onboard` 复制到项目的 `.kflow/reference/maintainer-notes.md`。维护 kflow 技能家族时需要反复查阅、但不适合放在各子技能正文里的说明。
...
新工作流定型后，在 `k-onboard/reference/system-overview.md` 的"技能分成四部分"和"场景路由"表里加一段索引，并登记新的目录位置。
...
如果 kflow 自己形成了稳定共享术语，应优先沉淀成共享 reference，而不是散落在多个子技能里重复定义。
...
- 共享说明优先放 `.kflow/reference/`，不要散落在各子技能里