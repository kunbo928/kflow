...
由 `k-onboard` 复制到项目的 `.kflow/reference/shared-conventions.md`。所有 kflow 子技能用项目相对路径 `.kflow/reference/shared-conventions.md` 引用本文件——跨子技能共享但不适合堆在单个技能里的规范的唯一权威版本。
...
├── brainstorm/            brainstorm 阶段 spike 实验代码区（k-brainstorm 临时产出）
...
- 项目注意事项入口固定为 `.kflow/attention.md`，所有 kflow 子技能启动前必须读取；跨平台 AI 通过 `AGENTS.md`（k-onboard 生成）发现 kflow 体系
...
**触发时谁负责**：`k-arch` 的 `backfill` / `update` 模式在 Phase 6 落盘前主动检查并搬迁；命中阈值时这次操作要把"本次新加 / 改的 + 已有同类全部"一起搬，并同步改 `ARCHITECTURE.md` 链接（搬迁本身要在 Phase 5 给用户 review，不偷偷做）。`check` 模式不主动搬迁，但发现 ≥6 仍平铺时在报告末尾列为观察项。

### 改目录结构

改 `k-onboard/reference/shared-conventions.md` 模板，新项目 onboard 时带上新版本；已有项目手动同步 `.kflow/reference/shared-conventions.md`。
...
- learning / trick / decision / explore 四类**统一写入 `.kflow/compound/`**
...
- 由 `k-feat-design` 在 design 确认通过后一次生成 `steps` + `checks`
- `k-feat-ff` **不生成** checklist（也不写 design / acceptance），是跳过 spec 流程直接写代码的超轻量通道；唯一留下的痕迹是动手后回写的 `{slug}-ff-note.md`（轻量回顾，参与 scoped-commit、可被 k-arch / k-req backfill 检索到）
...
`.kflow/roadmap/{slug}/{slug}-items.yaml` 是规划层和 feature 执行层的唯一接口。三个技能共同读写它——是 skill 都读写项目共享产物，不算耦合。

**items.yaml 状态机**：

```
planned  → in-progress  （k-feat-design 启动 feature 时改）
in-progress → done      （k-feat-accept 验收完成时改）
planned  → dropped      （k-roadmap update 模式，用户决定不做时改）
```

`done` / `dropped` 是终态。需要回退重做的新加一条 slug 略改的条目，不改终态。

**k-roadmap 的职责**：生成和维护 roadmap 主文档 + items.yaml；把 `planned` 改 `dropped`（用户放弃时）；不改 `in-progress` / `done`（feature 技能负责）。

**k-feat-design 的职责**（从 roadmap 起头时）：
...
feature-design / issue-analyze / issue-fix 动手前到 `.kflow/compound/` 搜已有沉淀：
...
子技能只补本阶段查询命令。完整搜索语法看 `.kflow/reference/tools.md`。

---

## 6. 归档类子技能共享守护规则

`k-learn` / `k-trick` / `k-decide` / `k-explore` 共享下面这组规则。子技能正文只写特有反模式，通用看这里：
...
4. **attention.md 检查**——写完后若沉淀暴露出"每次启动都该知道"的一两行硬约束，提示用户用 `k-note` 追加到 `.kflow/attention.md`；不要直接改 `AGENTS.md`（入口文件由 k-onboard 统一管理）
...
`k-feat-impl` 和 `k-issue-fix` 共用。AI 默认会往"大函数 / 大文件 / god class / 处处特殊分支"漂，这一节把漂移截在发生那一刻。
