...
3. **有测试能自证吗？** 目标代码已有运行时测试覆盖（单测 / 集成测）——没有就退完整 `k-refactor` 补刻画测试；fastforward 不现场补测试。完整测试口径看 `.kflow/reference/shared-testing.md`
...
- 跑测试（单元 / 集成），**必须有运行时测试通过**，不能只靠类型检查
- grep 检查旧引用是否清理干净（做了 Extract / Inline 这类）
- 改了前端状态逻辑跑已有测试 + 类型检查；**不做 UI 目视验证**——要 UI 目视就不该走 fastforward
...
默认**不建 `.kflow/refactors/` 目录**——fastforward 的价值就在不留存档。

例外：用户明确"这次要留个记录" → 建 `.kflow/refactors/{YYYY-MM-DD}-{slug}/{slug}-refactor-note.md`，内容就是上面那句汇报 + 一段"做了什么 / 为什么"。不写 design / checklist。
...
切回：触发 `k-refactor` 从 scan 开始。已改的部分要么提交保留、要么 `git restore` 回到干净状态再扫。
...
- **自证偷懒**：只跑类型检查不跑运行时测试；完全不 grep 旧引用
...
- `k-refactor/SKILL.md` — 完整 refactor 流程
- `k-refactor/reference/methods.md` — 完整方法库
