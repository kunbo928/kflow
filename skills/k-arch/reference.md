...
```bash
kflow search --dir .kflow/compound --filter "doc_type=decision|explore|learning" --query "{模块关键词}"
kflow search --dir .kflow/compound --filter doc_type=decision --filter status=active --query "{模块关键词}"
```
