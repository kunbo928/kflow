...
> 共享路径与命名约定看 `.kflow/reference/shared-paths.md`。产物写入 `.kflow/compound/`，命名 `YYYY-MM-DD-explore-{slug}.md`，frontmatter 带 `doc_type: explore`。
...
- 新建：写入 `.kflow/compound/YYYY-MM-DD-explore-{slug}.md`，frontmatter 带 `doc_type: explore`
- 更新：写回 Phase 1.5 定位的原文件 + `updated: YYYY-MM-DD`
- supersede：按 `shared-archive.md` supersede 规则；旧文档 `status: outdated` + `superseded-by`
...
kflow search --dir .kflow/compound --filter doc_type=explore --filter type=module-overview --filter status=active

# 归档后查重叠

kflow search --dir .kflow/compound --filter doc_type=explore --query "{关键词}" --json
```
...
> 归档类共享规则见 `shared-archive.md`。本技能特有反模式：
