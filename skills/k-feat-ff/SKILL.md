---
name: k-feat-ff
description: feature 流程的超轻量通道——不写 design / checklist 直接动手，但先指引 AI 查 kflow 知识库再开工。触发：用户说"快速模式"、"fastforward"、"别那么多步骤"、"直接开干"，且需求小到不值得走 design 流程。
---
...
- **`compound/`** — learning / trick / decision / explore 四类沉淀：
  ```bash
  kflow search --dir .kflow/compound --filter doc_type=learning --query "关键词"
  kflow search --dir .kflow/compound --filter doc_type=decision --query "关键词"
  kflow search --dir .kflow/compound --filter doc_type=trick --query "关键词"
  ```
