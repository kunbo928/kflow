---
name: k-feat
description: 实现新功能或功能改造。不用于纯 bug 修复（k-issue）、行为等价重构（k-refactor）、大需求拆解（k-roadmap）。
argument-hint: "[功能描述]"
---
...
- 按功能关键词检索 `.kflow/lessons/` 与项目文档；命中要报告来源路径。稳定结论按归属进入代码、测试、项目文档、ADR 或 lesson。
...
理解相关事实 → 实现 → 运行相称的验证 → 交付结果。普通任务不生成 kflow 产物，git diff、测试输出和交付说明就是证据。

## 可执行反馈闭环

Feature 使用目标行为 `red → green`：先选调用方与测试共同经过的真实接缝，建立一个会因目标行为尚未
实现而失败、且能重复运行的信号，再做最小完整实现，并用同一信号观察 green。信号优先是项目正常
测试目录中的行为测试；不适合自动化时使用可重复的集成探针、CLI 输出、类型契约或经 owner 确认的
人工步骤。信号必须能区分目标行为是否成立，不能只证明程序启动或代码通过 lint。

实现按 tracer bullet 切成可独立观察的窄行为切片：一条信号、一个最小实现、一次 green；随后才扩展
下一片。测试接缝存在真实取舍、需要新增公开接口或显著增加测试成本时才向 owner 确认，不把 seam
确认变成固定 gate。没有可信接缝时先改善结构或报告限制，不写只锁定内部实现的测试来伪造保障。

## 条件式原型

原型是降低不确定性的手段，不是独立任务入口。只有用户明确要求，或交互、接口、外部能力、性能假设
存在多个真实方向且直接实现的返工代价明显时才触发；相邻实现已给出明确答案的普通功能直接进入反馈
闭环。开始前在对话、既有 design 或 执行游标中写清：**决策问题、待验证假设、最低实现、观察项、
退出条件、处置方式**，一个原型只回答一个主要决策问题。

- 交互原型验证状态、流程与理解；技术 spike 验证 API、依赖、性能或可行性。诊断 harness 属于
  `k-issue`，不作为 Feature 原型。
- 默认放仓库外临时目录或项目既有 prototype 归宿；需要写入产品工作树、安装新依赖或产生代码外
  副作用时，先按对应写入、依赖和授权边界处理。
- 原型通过只证明假设或帮助 owner 选择，不等于功能完成。mock、硬编码、跳过的错误处理与生产差距
  必须显式列出；临时代码在决策后清理，保留时只留 artifact 指针与结论。
- 要复用原型代码时，把它当作未验证输入重新纳入 Feature：建立目标行为 red，按项目结构实现并通过
  完整 green 与相称验证；不得把探索产物静默晋升为产品代码。
...
跨会话且前述载体不存在时写入既有 执行游标的 `边界` 节，不为术语新建游标。未触发时不增加章节、
问题或产物。
...
- 项目已有测试设施时优先测试先行：先把验收场景写进正常测试目录并实际观察 red；red 与 green 必须使用同一命令。确实无法自动化时与用户确认验证方式。
