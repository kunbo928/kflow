...
guidedoc 产物**不在 `.kflow/` 下**——指南是面向外部读者的可发布产物，和 spec 工件分开。
...
```
kflow search --dir docs/dev --filter doc_type=dev-guide --filter status=current
kflow search --dir docs/user --filter doc_type=user-guide --filter component={feature-slug}
```
