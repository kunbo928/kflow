...
2. **上下文分级读取**：analysis 先读 frontmatter + 第 1 节定位 + 第 4 节影响面 + 第 5 节用户选定方案；report 先读复现步骤 / 期望行为；analysis 第 1 节定位的代码只读相关函数；`.kflow/attention.md` + 沉淀目录搜索。analysis/report 字段矛盾、修复范围不清、或要写 fix-note 时再读对应全文：
   - `kflow search --dir .kflow/compound --filter doc_type=trick --filter status=active --query "{关键词}"`——确认修复方式不违背已有库用法 / 模式
...
完整测试口径看 `.kflow/reference/shared-testing.md`。issue-fix 的 TDD 节奏：

1. **先写复现测试**——按 report 第 2 节复现步骤，写一个能稳定触发 bug 的测试（红）
2. **修复代码**——测试变绿
3. **补回归测试**——确认 analysis 第 4 节"潜在受害模块"的行为未被破坏
...
- [ ] **复现测试**——在修复前已写好，修复前红、修复后绿
- [ ] **复现步骤验证**——按 report 第 2 节走一遍，问题不再出现
- [ ] **期望行为验证**——report 第 3 节"期望行为"现在确实发生
- [ ] **影响面回归**——analysis 第 4 节"潜在受害模块"每个有回归测试覆盖
- [ ] **前端改动浏览器验证**（如涉及）——集成测试用真实浏览器，不能只 typecheck
- [ ] **全量测试通过**——所有已有测试 + 本次新增测试全绿
...
按 `shared-closeout.md`"scoped-commit"规则执行。本阶段：
...
1. 暴露了值得复用的坑点 → "沉淀 learning？（`k-learn`）"
2. 沉淀出长期约束 / 规约 / 技术决定 → "归档决定？（`k-decide`）"
3. 这个 bug 暴露了项目通用的硬约束 / 命令陷阱 / 环境设置（一两行能讲清、kflow 技能每次启动都该知道）→ "记到 attention.md？（`k-note`）"
4. 最后问是否代为提交。同意时按收尾提交规则执行

建议：把 issue 目录文件和代码改动放同一次提交方便追溯；"顺手发现"另开 `k-issue-report` 处理别塞这个 PR。
...
- 前端改动只 typecheck 就报通过——集成测试必须用真实浏览器
- 跳过复现测试直接修代码——"改完手工试了一下"不算
- 用户没明确说"修复完成"就结束
