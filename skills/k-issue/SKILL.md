---
name: k-issue
description: 诊断或修复 bug、报错、性能回退或既有行为异常。不用于新功能（k-feat）或行为等价重构（k-refactor）。
argument-hint: "[问题描述]"
---
...
- 按错误信息与相关模块关键词检索 `.kflow/lessons/` 与项目文档；这个坑可能踩过，命中要报告来源路径。稳定结论按归属进入代码、测试、项目文档、ADR 或 lesson。
...
- 诉求其实是新增能力而不是坏掉的行为时，转 `k-feat`，不在 issue 里偷做。

## 模式与证据

- 用户只要求定位、分析、解释或排查时，执行只诊断；只有用户明确要求或既有授权包含修复时才改动。
  诊断结论不扩大为修复授权。
- 只诊断不改产品代码、测试或配置，不创建 commit；跨会话 执行游标仍按既有规则处理。
...
- Issue 使用故障症状 `red → green`。先建立覆盖用户精确症状的可执行反馈闭环：同一命令或步骤能够
  在修复前观察 red、修复后观察 green，并尽量做到快速、确定、agent 可重复运行；不能只捕获附近错误。
  flaky 问题先循环、加压或固定随机性，提高并记录复现率，再以同一统计判据比较修复前后。
...
- 复现后逐项删减输入、调用方、配置、数据与步骤，得到仍覆盖原症状的最小复现；存在多个合理根因时
  才列可证伪假设，每个探针只检验一个预测。临时日志统一使用唯一 `[DEBUG-...]` 前缀，收尾时检索并清理。
