...
debug 的核心不是"把报错消掉"，而是找到行为为什么偏离预期，并确认修完后偏离消失。压力会诱人乱改：看到症状→猜一个原因→改一处→不行再改——系统会变成猜测残留物。
...
把反馈闭环当作诊断工具本身：尽量做到秒级、判据单义、环境隔离、agent 可独立重跑。获得 red 后，
逐项删除输入、调用方、配置、数据和步骤；删除任一剩余元素都会转 green 时，最小化才完成。原始完整
场景与最小场景都要保留可重跑指针，前者证明修复真实症状，后者适合固化回归。
...
修完清理临时埋点与一次性 harness；有复用价值的检查留下并说明。可复用的坑用 k-knowledge 沉淀。
