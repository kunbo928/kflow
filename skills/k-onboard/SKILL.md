...
- `.kflow/{requirements,roadmap,features,issues,refactors,brainstorms,compound}/`（建空目录，不放占位文件）
- `.kflow/attention.md`（最小骨架模板见同目录 `reference.md`）
- `.kflow/architecture/ARCHITECTURE.md`（占位模板见同目录 `reference.md`）
- `.kflow/tools/`（用 `cp -rf` / `Copy-Item -Recurse -Force` 整目录拷贝技能包 `tools/`，**不要 Read 再 Write**）
- `.kflow/reference/`（用 `cp -rf` 整目录拷贝技能包 `templates/`，**不要 Read 再 Write**）
...
**`.kflow/tools/`、`.kflow/reference/` 一律用技能包新版本覆盖**——这两个目录是技能包维护的共享资产，权威源在技能包（`tools/` 和 `templates/`），项目里的只是落盘副本。技能包升级后再跑 onboard 的目的之一就是刷新副本。
...
```bash
# macOS / Linux
cp -rf <技能源>/tools/.  .kflow/tools/
cp -rf <技能源>/templates/.             .kflow/reference/

# Windows PowerShell
Copy-Item -Recurse -Force <技能源>\tools\*  .kflow\tools\
Copy-Item -Recurse -Force <技能源>\templates\*             .kflow\reference\
```

不要：Read+Write 手工搬（截断 / 改缩进）、一个个 cp（多步骤多出错）、先比 diff（规则就是无条件覆盖）。

`<技能源>` 是 kflow 技能包所在目录（即包含 `tools/` 和 `templates/` 的目录）。拷完 `ls .kflow/tools/ .kflow/reference/` 验证。
