...
> 读取材料遵守 `.kflow/reference/shared-token-budget.md`。重构默认先锁范围、读目标文件和直接测试，不扫全仓库历史。
...
单函数 / 单组件 / 1-3 处优化 / 有测试可自证 / 不需要目视——走完整三阶段太重。触发 `k-refactor-ff`：直接识别、一次对齐、原地改、跑测试自证，不产 scan / design / checklist。
...
```
.kflow/refactors/{YYYY-MM-DD}-{slug}/
...
方法库在 `reference/methods.md`。扫描时先按 L1-L4 分类命中候选，再打开对应方法条目；只有候选横跨多层或要起草 design 时才通读方法库。
...
3. **识别前置依赖（测试覆盖为强制项）**——测试覆盖不够的条目默认前置"补刻画测试"；确实补不了时先把风险和原因列给用户确认，不静默硬改。改公开接口的前置"搜调用方"。完整测试口径看 `.kflow/reference/shared-testing.md`
...
2. **刻画测试先行**——每步开始前确认该步涉及的行为有测试覆盖：已有就复用，没有就先补刻画测试（捕获当前行为快照）。重构后刻画测试必须仍然全绿 = 行为等价得到证明。没有覆盖时不直接动手，先补测试；确实补不了就停下来让用户决定风险
3. **每步完成走验证**：
   - AI 自证：跑指定测试 / 类型检查 / lint / grep 无残留旧引用。通过了记 apply-notes 继续
   - HUMAN 验证：**停下来**汇报"第 N 步已完成，请在 {具体页面 / 操作} 目视确认，确认后我继续"。用户不明确说"继续"就不推进
4. **偏离当场记**——执行中发现方案没考虑的情况（如有个调用方在动态 import 里），**停下来汇报不发挥**。和用户对齐后追加到 apply-notes，必要时回阶段 2 改 design
5. **行为等价自检**——每步结束额外问"这一步有没有可能改了外部可观察行为？" 有怀疑就退回当步
...
- **覆盖率不够硬上**——没测试的模块直接改，"行为等价"只是口头承诺。先补刻画测试；补不了就停下来说明风险
...
- `k-refactor-ff/SKILL.md` — 小重构超轻量通道
