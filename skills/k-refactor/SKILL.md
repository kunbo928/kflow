---
name: k-refactor
description: 行为等价的重构、拆分、性能优化。会改变外部可观察行为的诉求走 k-feat 或 k-issue。
argument-hint: "[重构目标]"
---
...
- 按目标模块关键词检索 `.kflow/lessons/` 与项目文档；命中要报告来源路径。稳定结论按归属进入代码、测试、项目文档、ADR 或 lesson。
- 先确认诉求真是行为不变：一旦包含"顺便支持 X / 改成 Y"，把那部分拆出去转 `k-feat` 或 `k-issue`，不夹带。
- 结构好坏用**深度**衡量：小接口承载大行为是深，接口和实现一样复杂是浅；重构应让调用方用更少认知换更多能力，不为"看起来干净"搬家，不把模块越拆越碎。

## 发现目标

用户已指定模块、痛点或重构方向时，直接在该范围核实结构摩擦、真实接缝与等价基线，不先做全库架构扫描。
用户只给出“改善架构”一类开放目标时，先根据项目文档、ADR 与近期变更热点限定一个有界范围；历史只是发现线索，
不能单独证明值得重构。在该范围按以下问题寻找候选：

- **深度与杠杆**：接口增加的认知成本，是否真正封装了更多实现复杂度。
- **局部性**：理解或修改一个领域概念，是否需在过多小模块间往返，真正相关的行为是否能集中。
- **删除测试**：假想删掉模块；若复杂度只是泄漏或分散到多个调用方，它可能是值得加深的模块；若复杂度随之消失，它更可能是穿透层。
- **接缝真实性**：一个适配对象只能证明假想接缝；两个真实适配对象、已有消费者差异或可观察的变更压力，才能支撑抽象。
- **可测试性**：接口是否能作为真实测试面，还是只测了被抽出的纯函数，遗漏调用组合中的风险。

扫描结果只列有代码、测试或历史证据的候选；每项说明范围、摩擦、预期加深方向、局部性/杠杆/测试收益、ADR 冲突及
`Strong | Worth exploring | Speculative` 置信度，并给出一个首选。此时不把接口设计或推测当成既定结论；存在会改变结果的真实取舍时请
owner 选择，选定后再进入下方 `green → green` 流程。默认在对话中给简洁文本报告；只有关系确实需要可视化或用户要求时才增加图表或文件产物，
不默认打开预览。
...
范围时只给候选。用户已明确说“记住 / 更新 / 退役”时，同轮按 `k-knowledge` 处理，不重复确认。普通任务
...
整套 design、全量回归、独立 review、游标文件或 Roadmap final acceptance 路径。行数、文件数、
...
连续性需要不是风险门槛：跨会话、多人交接或用户要求留痕只增加单一临时 执行游标。普通改动选择
成本最低且足够权威的验证；已有定向测试足够时不叠加全量套件、浏览器 smoke 与独立 review。

## 硬门槛

- Refactor 使用行为基线 `green → green`：修改前在真实接缝运行现有权威测试；覆盖不足时先补
  characterization test 并观察 green，再小步改结构，每步运行最窄等价信号并保持 green。类型检查、
  lint 和快照不能单独证明运行时等价；它们只能补充对应维度的证据。
...
- 为比较重要接口可以做多个最小纸面或 throwaway 设计，但不把它们作为产品原型：只比较深度、调用方
  负担、真实接缝与可测试性，选定后仍在 green 基线上实施。若比较结果要求改变外部行为，停止并转
  `k-feat`；发现既有行为错误则转 `k-issue`。
...
- 需要跨会话继续时写 `.kflow/cursors/refactor-{slug}.md`（目标 / 现场 / 边界 / 证据 / 验收 / 状态与未决六节；游标文件一律带类型前缀）。完成后先在报告列毕业去向（结论进哪、lesson 沉哪，或明说无可毕业）再删除；目标位置不存在时在清单中建议落点请用户拍板，拍板前不删。用户要求留档则保留。
- 创建游标必须执行 `kflow cursor create k-refactor refactor-{slug} --skill k-refactor`，恢复先执行 `kflow cursor show refactor-{slug} --skill k-refactor`，交接或收尾前执行 `kflow cursor validate .kflow/cursors/refactor-{slug}.md --skill k-refactor`；不得手写创建或跳过 CLI 校验。普通零游标任务不为留痕而调用。
