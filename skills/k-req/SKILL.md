...
不适用：要写"技术上怎么搭" → `k-arch`；写单次 feature 方案 → `k-feat-design`；拍板长期规约 → `k-decide`；写外部"怎么用" → `k-guide`；大需求拆几轮做 → `k-roadmap`。
...
**按情况读**：可能承载这能力的 architecture doc（用于 `implemented_by`）；相关已有 feature 方案；compound 沉淀（`kflow search --dir .kflow/compound --query "{能力关键词}"`）。
