...
Feature 按需创建为 `feats/NN-readable-name.md`。编号只表示稳定展示和默认推进顺序；frontmatter 中的 `id`、`status`、`depends_on` 才定义身份和真实依赖。每个文件保存目标行为、范围与非目标、验收场景、测试契约、必要设计约束和最终结果。子 Feature 不再创建平行的独立 `feat-*` Work。
...
frontier 是依赖均已 accepted 的 proposed Feature。默认一次推进一个；只有 owner 选择并行且不存在共享归属接缝时才并行。每个 Feature 先达到 Spec Clear，再按 `k-feat` 的纪律执行：目标行为 red、最小纵向切片、同一信号 green、受影响回归、Spec 与 Standards 双轴审查。活动状态和未解决 finding 写入父级 `work.md`，稳定结果写入子 Feature 文件。
